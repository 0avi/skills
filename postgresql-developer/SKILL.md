...
- **When the database is not the problem.** If triage here shows the queries are fast and the time is going somewhere else, the Java-side costs are a separate skill: JDBC batching and prepared-statement pooling, connection-pool sizing, fetch size, transaction scope, and the JPA lazy-versus-eager and N+1 traps. Use [`java-performance-developer`](../java-performance-developer/SKILL.md), which owns establishing that the database *is* the constraint and defers query and schema work back to this skill.
